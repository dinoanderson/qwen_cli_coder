...
In addition to a project settings file, a project's `.qwen` directory can contain other project-specific files related to Qwen CLI's operation, such as:
...
- **`QWEN_API_KEY`** or **`DASHSCOPE_API_KEY`**:
  - Your API key for Qwen/DashScope services.
  - Required when using Qwen models as the AI provider.
  - Either variable name can be used (QWEN_API_KEY takes precedence).
  - Example: `export QWEN_API_KEY="YOUR_QWEN_API_KEY"`
- **`QWEN_BASE_URL`**:
  - Overrides the default DashScope API endpoint.
  - Default: `https://dashscope-intl.aliyuncs.com/api/v1`
  - Useful for users in regions that require specific endpoints.
  - Example: `export QWEN_BASE_URL="https://dashscope.aliyuncs.com/api/v1"`
- **`QWEN_ENABLE_THINKING`**:
  - Enables thinking mode for Qwen models (when supported).
  - Set to `true` to see the model's reasoning process.
  - Default: `false`
  - Example: `export QWEN_ENABLE_THINKING=true`
...
When `.qwen/sandbox.Dockerfile` exists, you can use `BUILD_SANDBOX` environment variable when running Qwen CLI to automatically build the custom sandbox image:
