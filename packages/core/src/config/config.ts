...
export class Config {
  ...
  private qwenMdFileCount: number;
  ...
  constructor(params: ConfigParameters) {
    ...
    this.qwenMdFileCount = params.qwenMdFileCount ?? 0;
  ...
  getQwenMdFileCount(): number {
    return this.qwenMdFileCount;
  }

  setQwenMdFileCount(count: number): void {
    this.qwenMdFileCount = count;
  }
...
export function createToolRegistry(config: Config): Promise<ToolRegistry> {
  ...
  registerCoreTool(ReadImageTool, targetDir, config);
...
export { DEFAULT_GEMINI_MODEL, DEFAULT_QWEN_MODEL } from './models.js';
