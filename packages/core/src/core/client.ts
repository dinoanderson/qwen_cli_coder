...
import { QwenChat } from './qwenChat.js';
...
export class QwenClient {
  ...
  private async startChat(extraHistory?: Content[]): Promise<QwenChat> {
    ...
    try {
      ...
      await reportError(
        error,
        'Error initializing Qwen chat session.',
        history,
        'startChat',
      );
  ...
  async *sendMessageStream(
  ...
  ): AsyncGenerator<ServerQwenStreamEvent, Turn> {
    if (!turns) {
    ...
    if (compressed) {
      yield { type: QwenEventType.ChatCompressed, value: compressed };
    }
    ...
    if (!turn.pendingToolCalls.length && signal && !signal.aborted) {
      // Temporarily disable checkNextSpeaker for Qwen due to conversation history formatting issues
      const authType = this.config.getContentGeneratorConfig()?.authType;
      if (authType !== 'qwen-dashscope') {
  ...
  private async handleFlashFallback(authType?: string): Promise<string | null> {
    // Qwen fallback is not implemented
    return null;
  }
}
