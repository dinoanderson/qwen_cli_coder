...
import { useQwenStream } from './hooks/useQwenStream.js';
...
const App = ({ config, settings, startupWarnings = [] }: AppProps) => {
  ...
  const [geminiMdFileCount, setQwenMdFileCount] = useState<number>(0);
  ...
  const performMemoryRefresh = useCallback(async () => {
    ...
    try {
      ...
      config.setQwenMdFileCount(fileCount);
      setQwenMdFileCount(fileCount);
...
⚡ You can switch authentication methods by typing /auth`,
  ...
  useEffect(() => {
    if (config) {
      setQwenMdFileCount(config.getQwenMdFileCount());
    }
  }, [config]);
