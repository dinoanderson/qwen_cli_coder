...
import { SandboxConfig } from '@qwen/qwen-cli-core';
...
export async function start_sandbox(
  ...
  if (process.env.QWEN_API_KEY) {
    args.push('--env', `QWEN_API_KEY=${process.env.QWEN_API_KEY}`);
  }
  ...
  if (process.env.QWEN_MODEL) {
    args.push('--env', `QWEN_MODEL=${process.env.QWEN_MODEL}`);
  }
  ...
  if (process.env.QWEN_CLI_INTEGRATION_TEST === 'true') {
    args.push('--user', 'root');
    userFlag = '--user root';
